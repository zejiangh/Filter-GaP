...
# Do data augumentation
class SSDTransformer_v2(object):
    """ SSD Data Augumentation, according to original paper
        Composed by several steps:
        Cropping
        Resize
        ...
        Jittering
    """
    def __init__(self, dboxes, size = (300, 300), val=True):
        ...
        self.trans_val_weak = transforms.Compose([
            transforms.Resize(self.size),
            transforms.ToTensor(),
            self.normalize,])
        ...
        self.trans_val_strong = transforms.Compose([
            transforms.Resize(self.size),
            transforms.RandomApply([color_jitter], p=0.8),
            transforms.RandomGrayscale(p=0.2),
            transforms.ToTensor(),
    ...
    def __call__(self, img, img_size, bbox=None, label=None, max_num=200):
        ...
        if self.val:
            ...
            bbox_out[:bbox.size(0), :] = bbox
            label_out[:label.size(0)] = label
            return [self.trans_val_weak(img), self.trans_val_strong(img)], img_size, bbox_out, label_out
